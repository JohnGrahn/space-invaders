...
export class EnemyController {
  ...
  moveEnemies() {
    let moveDown = false;
    const leftmostEnemy = this.enemies.reduce(
      (min, e) => (e.x < min.x ? e : min),
      this.enemies[0]
    );
    const rightmostEnemy = this.enemies.reduce(
      (max, e) => (e.x > max.x ? e : max),
      this.enemies[0]
    );

    if (
      (this.direction === 1 &&
        rightmostEnemy.x + rightmostEnemy.width + this.speed >
          this.canvasWidth) ||
